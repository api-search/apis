---
aid: avaloq:avaloq-banking-api
baseURL: https://api.avaloq.com
description: Core banking API providing access to account management, transactions, and customer data for wealth management and digital banking solutions. Supports over 7,500 REST API endpoints for comprehensive banking operations.
humanURL: https://developer.avaloq.com/
image: ''
layout: api
name: Avaloq Banking API
properties:
- type: Documentation
  url: https://developer.avaloq.com/
- type: GettingStarted
  url: https://developer.avaloq.com/web/developer-portal/getting-started/developing
- type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/avaloq/refs/heads/main/openapi/avaloq-banking-openapi.yml
provider_name: Avaloq
provider_slug: avaloq
slug: avaloq-banking-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: avaloq:avaloq-banking-api\nname: Avaloq Banking API\ndescription: Core banking API providing access to account management, transactions, and customer data\n  for wealth management and digital banking solutions. Supports over 7,500 REST API endpoints for comprehensive\n  banking operations.\nhumanURL: https://developer.avaloq.com/\nbaseURL: https://api.avaloq.com\ntags:\n- Accounts\n- Banking\n- Core Banking\n- Transactions\n- Wealth Management\nproperties:\n- type: Documentation\n  url: https://developer.avaloq.com/\n- type: GettingStarted\n  url: https://developer.avaloq.com/web/developer-portal/getting-started/developing\n- type: OpenAPI\n  url: https://raw.githubusercontent.com/api-evangelist/avaloq/refs/heads/main/openapi/avaloq-banking-openapi.yml\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/avaloq/refs/heads/main/apis.yml
tags:
- Accounts
- Banking
- Core Banking
- Transactions
- Wealth Management
---
