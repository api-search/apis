---
aid: autodesk:bim-360-api
baseURL: https://developer.api.autodesk.com
description: The BIM 360 API enables integration with the BIM 360 cloud-based construction management platform, providing access to project data, documents, issues, and workflows for greater customization and workflow automation.
humanURL: https://aps.autodesk.com/developer/overview/bim-360-api
image: ''
layout: api
name: Autodesk BIM 360 API
properties:
- type: Documentation
  url: https://aps.autodesk.com/developer/overview/bim-360-api
- type: APIReference
  url: https://aps.autodesk.com/en/docs/bim360/v1/reference/
- type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/autodesk/refs/heads/main/openapi/autodesk-bim360-openapi.yml
provider_name: Autodesk
provider_slug: autodesk
slug: bim-360-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: autodesk:bim-360-api\nname: Autodesk BIM 360 API\ndescription: The BIM 360 API enables integration with the BIM 360 cloud-based construction management\n  platform, providing access to project data, documents, issues, and workflows for greater customization\n  and workflow automation.\nhumanURL: https://aps.autodesk.com/developer/overview/bim-360-api\nbaseURL: https://developer.api.autodesk.com\ntags:\n- BIM\n- Construction\n- Project Management\nproperties:\n- type: Documentation\n  url: https://aps.autodesk.com/developer/overview/bim-360-api\n- type: APIReference\n  url: https://aps.autodesk.com/en/docs/bim360/v1/reference/\n- type: OpenAPI\n  url: https://raw.githubusercontent.com/api-evangelist/autodesk/refs/heads/main/openapi/autodesk-bim360-openapi.yml\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/autodesk/refs/heads/main/apis.yml
tags:
- BIM
- Construction
- Project Management
---
