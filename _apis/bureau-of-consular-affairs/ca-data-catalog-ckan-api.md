---
aid: bureau-of-consular-affairs:ca-data-catalog-ckan-api
baseURL: https://cadatacatalog.state.gov/api/3/action
description: The CA Data Catalog provides access to datasets from the Bureau of Consular Affairs via the CKAN API. It includes passport issuance statistics, visa issuance data, adoption statistics, and other consular affairs data. The CKAN API supports dataset search, retrieval, and resource downloads.
humanURL: https://cadatacatalog.state.gov/
image: ''
layout: api
name: Bureau of Consular Affairs Data Catalog (CKAN API)
properties:
- type: Documentation
  url: https://cadatacatalog.state.gov/dataset/
- type: DataAPI
  url: https://cadatacatalog.state.gov/api/3
provider_name: Bureau of Consular Affairs
provider_slug: bureau-of-consular-affairs
slug: ca-data-catalog-ckan-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: bureau-of-consular-affairs:ca-data-catalog-ckan-api\nname: Bureau of Consular Affairs Data Catalog (CKAN API)\ntags:\n- CKAN\n- Data Catalog\n- Federal Government\n- Open Data\nhumanURL: https://cadatacatalog.state.gov/\nbaseURL: https://cadatacatalog.state.gov/api/3/action\nproperties:\n- url: https://cadatacatalog.state.gov/dataset/\n  type: Documentation\n- url: https://cadatacatalog.state.gov/api/3\n  type: DataAPI\ndescription: The CA Data Catalog provides access to datasets from the Bureau of Consular Affairs via the\n  CKAN API. It includes passport issuance statistics, visa issuance data, adoption statistics, and other\n  consular affairs data. The CKAN API supports dataset search, retrieval, and resource downloads.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/bureau-of-consular-affairs/refs/heads/main/apis.yml
tags:
- CKAN
- Data Catalog
- Federal Government
- Open Data
---
