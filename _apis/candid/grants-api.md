---
aid: candid:grants-api
baseURL: https://api.candid.org/grants
description: Access to Candid's global grants dataset — summary statistics, funders, recipients, and individual transaction records. Useful for philanthropic benchmarking, funder research, and grant-market intelligence.
humanURL: https://developer.candid.org/reference/welcome
image: ''
layout: api
name: Candid Grants API
properties:
- type: Documentation
  url: https://developer.candid.org/reference/welcome
- type: Portal
  url: https://developer.candid.org/
provider_name: Candid
provider_slug: candid
slug: grants-api
source_yaml: "aid: candid:grants-api\nname: Candid Grants API\ndescription: Access to Candid's global grants dataset — summary statistics, funders, recipients, and individual\n  transaction records. Useful for philanthropic benchmarking, funder research, and grant-market intelligence.\nhumanURL: https://developer.candid.org/reference/welcome\nbaseURL: https://api.candid.org/grants\ntags:\n- Grants\n- Funders\n- Recipients\n- Transactions\nproperties:\n- type: Documentation\n  url: https://developer.candid.org/reference/welcome\n- type: Portal\n  url: https://developer.candid.org/\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/candid/refs/heads/main/apis.yml
tags:
- Grants
- Funders
- Recipients
- Transactions
---
