---
aid: candid:nonprofit-eligibility-api
baseURL: https://api.candid.org/eligibility
description: Evaluates whether a given nonprofit is eligible to receive a grant or donation based on configurable rules — IRS status, country, OFAC, custom program criteria — to automate grantmaking and giving workflows.
humanURL: https://developer.candid.org/reference/welcome
image: ''
layout: api
name: Candid Nonprofit Eligibility API
properties:
- type: Documentation
  url: https://developer.candid.org/reference/welcome
- type: Portal
  url: https://developer.candid.org/
provider_name: Candid
provider_slug: candid
slug: nonprofit-eligibility-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: candid:nonprofit-eligibility-api\nname: Candid Nonprofit Eligibility API\ndescription: Evaluates whether a given nonprofit is eligible to receive a grant or donation based on configurable\n  rules — IRS status, country, OFAC, custom program criteria — to automate grantmaking and giving workflows.\nhumanURL: https://developer.candid.org/reference/welcome\nbaseURL: https://api.candid.org/eligibility\ntags:\n- Eligibility\n- Grantmaking\n- Compliance\nproperties:\n- type: Documentation\n  url: https://developer.candid.org/reference/welcome\n- type: Portal\n  url: https://developer.candid.org/\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/candid/refs/heads/main/apis.yml
tags:
- Eligibility
- Grantmaking
- Compliance
---
