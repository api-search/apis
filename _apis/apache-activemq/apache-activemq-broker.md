---
aid: apache-activemq:apache-activemq-broker
baseURL: ''
description: The ActiveMQ Classic broker provides high-performance asynchronous messaging through multiple protocol interfaces including OpenWire, AMQP, STOMP, and MQTT. It includes a web-based management console, message persistence, network of brokers for load distribution, and high availability configurations.
humanURL: https://activemq.apache.org/components/classic/documentation
image: ''
layout: api
name: Apache ActiveMQ Broker
properties:
- type: Documentation
  url: https://activemq.apache.org/components/classic/documentation
- type: GettingStarted
  url: https://activemq.apache.org/components/classic/documentation/getting-started
provider_name: Apache ActiveMQ
provider_slug: apache-activemq
slug: apache-activemq-broker
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: apache-activemq:apache-activemq-broker\nname: Apache ActiveMQ Broker\ndescription: The ActiveMQ Classic broker provides high-performance asynchronous messaging through multiple\n  protocol interfaces including OpenWire, AMQP, STOMP, and MQTT. It includes a web-based management console,\n  message persistence, network of brokers for load distribution, and high availability configurations.\nhumanURL: https://activemq.apache.org/components/classic/documentation\ntags:\n- Broker\n- Messaging\nproperties:\n- type: Documentation\n  url: https://activemq.apache.org/components/classic/documentation\n- type: GettingStarted\n  url: https://activemq.apache.org/components/classic/documentation/getting-started\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/apache-activemq/refs/heads/main/apis.yml
tags:
- Broker
- Messaging
---
