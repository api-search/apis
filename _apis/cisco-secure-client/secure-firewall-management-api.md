---
aid: cisco-secure-client:secure-firewall-management-api
baseURL: ''
description: The Cisco Secure Firewall Management Center API configures remote-access VPN gateways, group policies, and Secure Client profiles distributed to endpoints. Authentication uses a token generated via the generatetoken endpoint and passed as the X-auth-access-token header on subsequent calls.
humanURL: https://developer.cisco.com/docs/secure-firewall-management-center-api/
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Cisco Secure Firewall Management Center API
properties:
- type: Documentation
  url: https://developer.cisco.com/docs/secure-firewall-management-center-api/
provider_name: Cisco Secure Client
provider_slug: cisco-secure-client
slug: secure-firewall-management-api
source_yaml: "aid: cisco-secure-client:secure-firewall-management-api\nname: Cisco Secure Firewall Management Center API\ntags:\n- ASA\n- Firewall\n- FTD\n- Management\n- VPN\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nhumanURL: https://developer.cisco.com/docs/secure-firewall-management-center-api/\nproperties:\n- url: https://developer.cisco.com/docs/secure-firewall-management-center-api/\n  type: Documentation\ndescription: The Cisco Secure Firewall Management Center API configures remote-access VPN gateways, group\n  policies, and Secure Client profiles distributed to endpoints. Authentication uses a token generated\n  via the generatetoken endpoint and passed as the X-auth-access-token header on subsequent calls.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cisco-secure-client/refs/heads/main/apis.yml
tags:
- ASA
- Firewall
- FTD
- Management
- VPN
---
