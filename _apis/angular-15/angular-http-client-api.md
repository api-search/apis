---
aid: angular-15:angular-http-client-api
baseURL: https://angular.io/api/common/http
description: Angular 15 HTTP client with functional interceptors support as a lighter alternative to class-based interceptors.
humanURL: https://v15.angular.io/api/common/http
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Angular 15 HTTP Client API
properties:
- type: Documentation
  url: https://v15.angular.io/api/common/http
provider_name: Angular 15
provider_slug: angular-15
slug: angular-http-client-api
source_yaml: "aid: angular-15:angular-http-client-api\nname: Angular 15 HTTP Client API\ntags:\n- HTTP\n- Interceptors\n- REST\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://angular.io/api/common/http\nhumanURL: https://v15.angular.io/api/common/http\nproperties:\n- url: https://v15.angular.io/api/common/http\n  type: Documentation\ndescription: Angular 15 HTTP client with functional interceptors support as a lighter alternative to class-based\n  interceptors.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/angular-15/refs/heads/main/apis.yml
tags:
- HTTP
- Interceptors
- REST
---
