---
aid: brocade-communications-systems:fabric-os-rest-api
baseURL: https://{switch-ip}/rest
description: The Brocade Fabric OS REST API provides programmable management of Brocade SAN switches and directors running Fabric OS. YANG-based REST modules support chassis, port, zoning, security, and performance configuration and monitoring for Fibre Channel SAN environments.
humanURL: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x.html
image: ''
layout: api
name: Brocade Fabric OS REST API
properties:
- type: Documentation
  url: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x.html
- type: GettingStarted
  url: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x/v26395730/v24190001.html
provider_name: Brocade Communications Systems
provider_slug: brocade-communications-systems
slug: fabric-os-rest-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: brocade-communications-systems:fabric-os-rest-api\nname: Brocade Fabric OS REST API\ndescription: The Brocade Fabric OS REST API provides programmable management of Brocade SAN switches and\n  directors running Fabric OS. YANG-based REST modules support chassis, port, zoning, security, and performance\n  configuration and monitoring for Fibre Channel SAN environments.\nhumanURL: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x.html\nbaseURL: https://{switch-ip}/rest\ntags:\n- Fabric OS\n- Fibre Channel\n- Network Management\n- SAN\nproperties:\n- type: Documentation\n  url: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x.html\n- type: GettingStarted\n  url: https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-rest-api/9-2-x/v26395730/v24190001.html\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/brocade-communications-systems/refs/heads/main/apis.yml
tags:
- Fabric OS
- Fibre Channel
- Network Management
- SAN
---
