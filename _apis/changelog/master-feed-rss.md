---
aid: changelog:master-feed-rss
baseURL: https://changelog.com
description: The Changelog Master Feed aggregates all Changelog podcast shows into a single RSS feed. This is the one-stop subscription for all developer-focused audio content produced by Changelog Media, including The Changelog podcast and any other active shows in the network.
humanURL: https://changelog.com/master
image: ''
layout: api
name: Changelog Master Feed RSS
properties:
- type: RSSFeed
  url: https://changelog.com/master/feed
- type: Documentation
  url: https://changelog.com/master
provider_name: Changelog
provider_slug: changelog
slug: master-feed-rss
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: changelog:master-feed-rss\nname: Changelog Master Feed RSS\ndescription: The Changelog Master Feed aggregates all Changelog podcast shows into a single RSS feed.\n  This is the one-stop subscription for all developer-focused audio content produced by Changelog Media,\n  including The Changelog podcast and any other active shows in the network.\nhumanURL: https://changelog.com/master\nbaseURL: https://changelog.com\ntags:\n- Aggregated\n- Master Feed\n- Podcast\n- RSS\nproperties:\n- type: RSSFeed\n  url: https://changelog.com/master/feed\n- type: Documentation\n  url: https://changelog.com/master\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/changelog/refs/heads/main/apis.yml
tags:
- Aggregated
- Master Feed
- Podcast
- RSS
---
