---
aid: anthropic:anthropic-admin-api
baseURL: ''
description: Manage administrative functions.
humanURL: https://docs.anthropic.com/en/api/admin-api/users/get-user
image: ''
layout: api
name: Anthropic Admin API
properties:
- type: Documentation
  url: https://docs.anthropic.com/en/api/admin-api/users/get-user
- type: OpenAPI
  url: https://raw.githubusercontent.com/api-evangelist/anthropic/refs/heads/main/properties/anthropic-admin-api-openapi.yml
provider_name: Anthropic
provider_slug: anthropic
slug: anthropic-admin-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: anthropic:anthropic-admin-api\nname: Anthropic Admin API\ntags:\n- Administrative\n- AI\n- Artificial Intelligence\nhumanURL: https://docs.anthropic.com/en/api/admin-api/users/get-user\nproperties:\n- url: https://docs.anthropic.com/en/api/admin-api/users/get-user\n  type: Documentation\n- url: properties/anthropic-admin-api-openapi.yml\n  type: OpenAPI\ndescription: Manage administrative functions.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/anthropic/refs/heads/main/apis.yml
tags:
- Administrative
- AI
- Artificial Intelligence
---
