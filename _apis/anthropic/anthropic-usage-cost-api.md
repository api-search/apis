---
aid: anthropic:anthropic-usage-cost-api
baseURL: ''
description: Programmatically access your organization's API usage and cost data with the Usage & Cost Admin API. The Usage API tracks token consumption across your organization with detailed breakdowns by model, workspace, and service tier via the /v1/organizations/usage_report/messages endpoint. The Cost API retrieves service-level cost breakdowns in USD via the /v1/organizations/cost_report endpoint. Requires an Admin API key.
humanURL: https://docs.anthropic.com/en/api/usage-cost-api
image: ''
layout: api
name: Anthropic Usage & Cost API
properties:
- type: Documentation
  url: https://docs.anthropic.com/en/api/usage-cost-api
provider_name: Anthropic
provider_slug: anthropic
slug: anthropic-usage-cost-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: anthropic:anthropic-usage-cost-api\nname: Anthropic Usage & Cost API\ntags:\n- Administration\n- AI\n- Artificial Intelligence\n- Cost\n- Usage\nhumanURL: https://docs.anthropic.com/en/api/usage-cost-api\nproperties:\n- url: https://docs.anthropic.com/en/api/usage-cost-api\n  type: Documentation\ndescription: Programmatically access your organization's API usage and cost data with the Usage & Cost\n  Admin API. The Usage API tracks token consumption across your organization with detailed breakdowns\n  by model, workspace, and service tier via the /v1/organizations/usage_report/messages endpoint. The\n  Cost API retrieves service-level cost breakdowns in USD via the /v1/organizations/cost_report endpoint.\n  Requires an Admin API key.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/anthropic/refs/heads/main/apis.yml
tags:
- Administration
- AI
- Artificial Intelligence
- Cost
- Usage
---
