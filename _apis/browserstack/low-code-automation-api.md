---
aid: browserstack:low-code-automation-api
baseURL: ''
description: The BrowserStack Low Code Automation REST API enables triggering test suite executions and retrieving build statuses for CI/CD pipeline integration. It also supports exporting low-code tests as code in .side and Nightwatch.js formats.
humanURL: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/rest-api
image: ''
layout: api
name: BrowserStack Low Code Automation API
properties:
- type: Documentation
  url: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/rest-api
- type: Reference
  url: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/run-tests-api
- type: Getting Started
  url: https://www.browserstack.com/docs/low-code-automation
provider_name: BrowserStack
provider_slug: browserstack
slug: low-code-automation-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: browserstack:low-code-automation-api\nname: BrowserStack Low Code Automation API\ntags:\n- Automation\n- CI/CD\n- Low Code\n- Testing\nhumanURL: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/rest-api\nproperties:\n- url: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/rest-api\n  type: Documentation\n- url: https://www.browserstack.com/docs/low-code-automation/cicd-integrations/run-tests-api\n  type: Reference\n- url: https://www.browserstack.com/docs/low-code-automation\n  type: Getting Started\ndescription: The BrowserStack Low Code Automation REST API enables triggering test suite executions and\n  retrieving build statuses for CI/CD pipeline integration. It also supports exporting low-code tests\n  as code in .side and Nightwatch.js formats.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/browserstack/refs/heads/main/apis.yml
tags:
- Automation
- CI/CD
- Low Code
- Testing
---
