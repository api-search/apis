---
aid: browserstack:app-accessibility-testing-api
baseURL: ''
description: The BrowserStack App Accessibility REST API enables programmatic access to accessibility data for mobile app projects and builds. It provides endpoints for retrieving accessibility results from automated and manual tests, including project lists, build details, test cases, and issue-level analysis.
humanURL: https://www.browserstack.com/docs/app-accessibility/api-reference/introduction
image: ''
layout: api
name: BrowserStack App Accessibility Testing API
properties:
- type: Documentation
  url: https://www.browserstack.com/docs/app-accessibility/api-reference/introduction
- type: Getting Started
  url: https://www.browserstack.com/docs/app-accessibility/overview/introduction
provider_name: BrowserStack
provider_slug: browserstack
slug: app-accessibility-testing-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: browserstack:app-accessibility-testing-api\nname: BrowserStack App Accessibility Testing API\ntags:\n- Accessibility\n- Mobile Testing\n- Testing\nhumanURL: https://www.browserstack.com/docs/app-accessibility/api-reference/introduction\nproperties:\n- url: https://www.browserstack.com/docs/app-accessibility/api-reference/introduction\n  type: Documentation\n- url: https://www.browserstack.com/docs/app-accessibility/overview/introduction\n  type: Getting Started\ndescription: The BrowserStack App Accessibility REST API enables programmatic access to accessibility\n  data for mobile app projects and builds. It provides endpoints for retrieving accessibility results\n  from automated and manual tests, including project lists, build details, test cases, and issue-level\n  analysis.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/browserstack/refs/heads/main/apis.yml
tags:
- Accessibility
- Mobile Testing
- Testing
---
