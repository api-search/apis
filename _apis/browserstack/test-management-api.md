---
aid: browserstack:test-management-api
baseURL: ''
description: The BrowserStack Test Management API provides REST access to manage test projects, folders, test cases, test runs, test plans, test results, attachments, configurations, and custom fields for organizing and executing quality assurance workflows.
humanURL: https://www.browserstack.com/docs/test-management/api-reference/introduction
image: ''
layout: api
name: BrowserStack Test Management API
properties:
- type: Documentation
  url: https://www.browserstack.com/docs/test-management/api-reference/introduction
- type: Authentication
  url: https://www.browserstack.com/docs/test-management/api-reference/authentication
- type: Rate Limits
  url: https://www.browserstack.com/docs/test-management/api-reference/rate-limit-for-api-calls
provider_name: BrowserStack
provider_slug: browserstack
slug: test-management-api
source_yaml: "aid: browserstack:test-management-api\nname: BrowserStack Test Management API\ntags:\n- QA\n- Test Management\n- Testing\nhumanURL: https://www.browserstack.com/docs/test-management/api-reference/introduction\nproperties:\n- url: https://www.browserstack.com/docs/test-management/api-reference/introduction\n  type: Documentation\n- url: https://www.browserstack.com/docs/test-management/api-reference/authentication\n  type: Authentication\n- url: https://www.browserstack.com/docs/test-management/api-reference/rate-limit-for-api-calls\n  type: Rate Limits\ndescription: The BrowserStack Test Management API provides REST access to manage test projects, folders,\n  test cases, test runs, test plans, test results, attachments, configurations, and custom fields for\n  organizing and executing quality assurance workflows.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/browserstack/refs/heads/main/apis.yml
tags:
- QA
- Test Management
- Testing
---
