---
aid: centers-for-medicare-and-medicaid-services:cms-bcda
baseURL: https://api.bcda.cms.gov/api/v2/
description: The Beneficiary Claims Data API (BCDA) is a Bulk FHIR API that delivers Medicare Part A, B, and D claims data to Medicare Shared Savings Program ACOs, ACO REACH participants, and other Alternative Payment Model participants for their attributed and assignable beneficiaries.
humanURL: https://bcda.cms.gov/
image: ''
layout: api
name: CMS Beneficiary Claims Data API (BCDA)
properties:
- type: Website
  url: https://bcda.cms.gov/
- type: Documentation
  url: https://bcda.cms.gov/guide.html
- type: Sandbox
  url: https://sandbox.bcda.cms.gov/
provider_name: Centers for Medicare and Medicaid Services
provider_slug: centers-for-medicare-and-medicaid-services
slug: cms-bcda
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: centers-for-medicare-and-medicaid-services:cms-bcda\nname: CMS Beneficiary Claims Data API (BCDA)\ntags:\n- ACO\n- BCDA\n- Bulk FHIR\n- Claims\n- Medicare\n- Shared Savings\nhumanURL: https://bcda.cms.gov/\nbaseURL: https://api.bcda.cms.gov/api/v2/\nproperties:\n- url: https://bcda.cms.gov/\n  type: Website\n- url: https://bcda.cms.gov/guide.html\n  type: Documentation\n- url: https://sandbox.bcda.cms.gov/\n  type: Sandbox\ndescription: The Beneficiary Claims Data API (BCDA) is a Bulk FHIR API that delivers Medicare Part A,\n  B, and D claims data to Medicare Shared Savings Program ACOs, ACO REACH participants, and other Alternative\n  Payment Model participants for their attributed and assignable beneficiaries.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/centers-for-medicare-and-medicaid-services/refs/heads/main/apis.yml
tags:
- ACO
- BCDA
- Bulk FHIR
- Claims
- Medicare
- Shared Savings
---
