---
aid: chemius:erp-api
baseURL: ''
description: API for integrating Chemius product, SDS, TDS, and label data with enterprise ERP systems for synchronized chemical product information.
humanURL: https://www.chemius.net/chemius-api/
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Chemius ERP Integration API
properties:
- type: Documentation
  url: https://www.chemius.net/chemius-api/
provider_name: Chemius
provider_slug: chemius
slug: erp-api
source_yaml: "aid: chemius:erp-api\nname: Chemius ERP Integration API\ndescription: API for integrating Chemius product, SDS, TDS, and label data with enterprise ERP systems\n  for synchronized chemical product information.\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nhumanURL: https://www.chemius.net/chemius-api/\ntags:\n- ERP\n- Integration\nproperties:\n- type: Documentation\n  url: https://www.chemius.net/chemius-api/\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/chemius/refs/heads/main/apis.yml
tags:
- ERP
- Integration
---
