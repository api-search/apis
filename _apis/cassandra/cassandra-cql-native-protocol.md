---
aid: cassandra:cassandra-cql-native-protocol
baseURL: ''
description: Cassandra Query Language (CQL) is the primary interface to Apache Cassandra. Clients speak the binary CQL native protocol over TCP (default port 9042). Official drivers are maintained for Java, Python, Go, C/C++, C#, Node.js, Ruby, and Rust.
humanURL: https://cassandra.apache.org/doc/latest/cassandra/cql/
image: ''
layout: api
name: Apache Cassandra CQL Native Protocol
properties:
- type: Documentation
  url: https://cassandra.apache.org/doc/latest/cassandra/cql/
- type: Specification
  url: https://github.com/apache/cassandra/blob/trunk/doc/native_protocol_v5.spec
- type: Reference
  url: https://cassandra.apache.org/doc/latest/cassandra/cql/ddl.html
provider_name: Apache Cassandra
provider_slug: cassandra
slug: cassandra-cql-native-protocol
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: cassandra:cassandra-cql-native-protocol\nname: Apache Cassandra CQL Native Protocol\ntags:\n- CQL\n- Database\n- Native Protocol\n- Query\nhumanURL: https://cassandra.apache.org/doc/latest/cassandra/cql/\nproperties:\n- url: https://cassandra.apache.org/doc/latest/cassandra/cql/\n  type: Documentation\n- url: https://github.com/apache/cassandra/blob/trunk/doc/native_protocol_v5.spec\n  type: Specification\n- url: https://cassandra.apache.org/doc/latest/cassandra/cql/ddl.html\n  type: Reference\ndescription: Cassandra Query Language (CQL) is the primary interface to Apache Cassandra. Clients speak\n  the binary CQL native protocol over TCP (default port 9042). Official drivers are maintained for Java,\n  Python, Go, C/C++, C#, Node.js, Ruby, and Rust.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cassandra/refs/heads/main/apis.yml
tags:
- CQL
- Database
- Native Protocol
- Query
---
