---
aid: cloudbees:cd-ro
baseURL: https://example-cd.cloudbees.com/rest/v1.0
description: The CloudBees CD/RO (Continuous Delivery / Release Orchestration) REST API exposes resources for pipelines, releases, environments, applications, deployments, projects and resources. Operations cover modelling deployment pipelines, launching releases, tracking stages, managing environments and inventories, and integrating with Jenkins and other CI tools.
humanURL: https://docs.cloudbees.com/docs/cloudbees-cd/latest/api/
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: CloudBees CD/RO REST API
properties:
- type: Documentation
  url: https://docs.cloudbees.com/docs/cloudbees-cd/latest/api/
- type: Jenkins Plugin
  url: https://docs.cloudbees.com/plugins/cd/ec-jenkins
provider_name: CloudBees
provider_slug: cloudbees
slug: cd-ro
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: cloudbees:cd-ro\nname: CloudBees CD/RO REST API\ntags:\n- Continuous Delivery\n- DevOps\n- Pipelines\n- Release Orchestration\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://example-cd.cloudbees.com/rest/v1.0\nhumanURL: https://docs.cloudbees.com/docs/cloudbees-cd/latest/api/\nproperties:\n- url: https://docs.cloudbees.com/docs/cloudbees-cd/latest/api/\n  type: Documentation\n- url: https://docs.cloudbees.com/plugins/cd/ec-jenkins\n  type: Jenkins Plugin\ndescription: The CloudBees CD/RO (Continuous Delivery / Release Orchestration) REST API exposes resources\n  for pipelines, releases, environments, applications, deployments, projects and resources. Operations\n  cover modelling deployment pipelines, launching releases, tracking stages, managing environments and\n  inventories, and integrating with Jenkins and other CI tools.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cloudbees/refs/heads/main/apis.yml
tags:
- Continuous Delivery
- DevOps
- Pipelines
- Release Orchestration
---
