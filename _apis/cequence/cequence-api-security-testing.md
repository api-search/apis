---
aid: cequence:cequence-api-security-testing
baseURL: ''
description: API Security Testing extends Cequence into shift-left, performing pre-production OpenAPI conformance and vulnerability testing against the OWASP API Security Top 10, feeding results back into Sentinel and Spartan for continuous lifecycle protection.
humanURL: https://www.cequence.ai/blog/cequence-product-news/announcing-unified-api-protection-v2-0/
image: ''
layout: api
name: Cequence API Security Testing
properties:
- type: Announcement
  url: https://www.cequence.ai/blog/cequence-product-news/announcing-unified-api-protection-v2-0/
- type: Website
  url: https://www.cequence.ai/products/
provider_name: Cequence Security
provider_slug: cequence
slug: cequence-api-security-testing
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: cequence:cequence-api-security-testing\nname: Cequence API Security Testing\ntags:\n- Conformance\n- DAST\n- OWASP API Top 10\n- Pre-Production\n- Testing\nhumanURL: https://www.cequence.ai/blog/cequence-product-news/announcing-unified-api-protection-v2-0/\nproperties:\n- url: https://www.cequence.ai/blog/cequence-product-news/announcing-unified-api-protection-v2-0/\n  type: Announcement\n- url: https://www.cequence.ai/products/\n  type: Website\ndescription: API Security Testing extends Cequence into shift-left, performing pre-production OpenAPI\n  conformance and vulnerability testing against the OWASP API Security Top 10, feeding results back into\n  Sentinel and Spartan for continuous lifecycle protection.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cequence/refs/heads/main/apis.yml
tags:
- Conformance
- DAST
- OWASP API Top 10
- Pre-Production
- Testing
---
