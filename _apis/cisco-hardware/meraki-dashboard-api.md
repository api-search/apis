---
aid: cisco-hardware:meraki-dashboard-api
baseURL: https://api.meraki.com/api/v1
description: The Meraki Dashboard API is a RESTful interface for cloud-managed Meraki hardware including switches, wireless access points, security appliances, cameras, and sensors. Authentication uses an API key passed in the X-Cisco-Meraki-API-Key header. All endpoints return JSON. The API is fully versioned and an OpenAPI specification is published live at the Meraki dashboard URL.
humanURL: https://developer.cisco.com/meraki/
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Cisco Meraki Dashboard API
properties:
- type: Documentation
  url: https://developer.cisco.com/meraki/api-latest/
- type: OpenAPI
  url: https://api.meraki.com/api/v1/openapiSpec
- type: Getting Started
  url: https://developer.cisco.com/meraki/api/getting-started/
provider_name: Cisco Hardware
provider_slug: cisco-hardware
slug: meraki-dashboard-api
source_yaml: "aid: cisco-hardware:meraki-dashboard-api\nname: Cisco Meraki Dashboard API\ntags:\n- Cloud Managed\n- Dashboard\n- Switching\n- Wireless\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://api.meraki.com/api/v1\nhumanURL: https://developer.cisco.com/meraki/\nproperties:\n- url: https://developer.cisco.com/meraki/api-latest/\n  type: Documentation\n- url: https://api.meraki.com/api/v1/openapiSpec\n  type: OpenAPI\n- url: https://developer.cisco.com/meraki/api/getting-started/\n  type: Getting Started\ndescription: The Meraki Dashboard API is a RESTful interface for cloud-managed Meraki hardware including\n  switches, wireless access points, security appliances, cameras, and sensors. Authentication uses an\n  API key passed in the X-Cisco-Meraki-API-Key header. All endpoints return JSON. The API is fully versioned\n  and an OpenAPI specification is published live at the Meraki dashboard URL.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cisco-hardware/refs/heads/main/apis.yml
tags:
- Cloud Managed
- Dashboard
- Switching
- Wireless
---
