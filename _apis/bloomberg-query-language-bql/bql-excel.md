---
aid: bloomberg-query-language-bql:bql-excel
baseURL: https://bloomberg.com/bql-excel
description: Use Bloomberg Query Language directly in Microsoft Excel to execute complex data queries and retrieve computed results in spreadsheet cells. Supports multi-row outputs, time series, and calculated fields.
humanURL: https://www.bloomberg.com/professional/solution/bloomberg-excel/
image: ''
layout: api
name: BQL in Bloomberg Excel Add-in
properties:
- type: Documentation
  url: https://www.bloomberg.com/professional/solution/bloomberg-excel/
provider_name: Bloomberg Query Language (BQL)
provider_slug: bloomberg-query-language-bql
slug: bql-excel
source_yaml: "aid: bloomberg-query-language-bql:bql-excel\nname: BQL in Bloomberg Excel Add-in\ndescription: Use Bloomberg Query Language directly in Microsoft Excel to execute complex data queries\n  and retrieve computed results in spreadsheet cells. Supports multi-row outputs, time series, and calculated\n  fields.\nhumanURL: https://www.bloomberg.com/professional/solution/bloomberg-excel/\nbaseURL: https://bloomberg.com/bql-excel\ntags:\n- BQL\n- Excel\n- Spreadsheet\n- Formulas\nproperties:\n- type: Documentation\n  url: https://www.bloomberg.com/professional/solution/bloomberg-excel/\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/bloomberg-query-language-bql/refs/heads/main/apis.yml
tags:
- BQL
- Excel
- Spreadsheet
- Formulas
---
