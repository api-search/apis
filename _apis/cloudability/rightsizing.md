---
aid: cloudability:rightsizing
baseURL: https://api.cloudability.com/v3/rightsizing
description: The Rightsizing API surfaces machine-learning generated downsizing, modernization and termination recommendations for AWS EC2, RDS, EBS, Azure VMs and disks, and Google Compute Engine instances, including estimated savings, confidence, and supporting utilization metrics.
humanURL: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Cloudability Rightsizing Recommendations API
properties:
- type: Documentation
  url: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas
provider_name: Cloudability
provider_slug: cloudability
slug: rightsizing
source_yaml: "aid: cloudability:rightsizing\nname: Cloudability Rightsizing Recommendations API\ntags:\n- Cost Optimization\n- Recommendations\n- Rightsizing\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://api.cloudability.com/v3/rightsizing\nhumanURL: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas\nproperties:\n- url: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas\n  type: Documentation\ndescription: The Rightsizing API surfaces machine-learning generated downsizing, modernization and termination\n  recommendations for AWS EC2, RDS, EBS, Azure VMs and disks, and Google Compute Engine instances, including\n  estimated savings, confidence, and supporting utilization metrics.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cloudability/refs/heads/main/apis.yml
tags:
- Cost Optimization
- Recommendations
- Rightsizing
---
