---
aid: cloudability:business-mappings
baseURL: https://api.cloudability.com/v3/business-mappings
description: Business Mappings define rule-based dimensions that allocate spend to cost centers, products, environments, or applications. The API lets callers list, create, update and delete mappings, manage rule order and statements, and preview the resulting allocation against billing data.
humanURL: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Cloudability Business Mappings API
properties:
- type: Documentation
  url: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas
provider_name: Cloudability
provider_slug: cloudability
slug: business-mappings
source_yaml: "aid: cloudability:business-mappings\nname: Cloudability Business Mappings API\ntags:\n- Allocation\n- Business Mappings\n- Showback\n- Tagging\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://api.cloudability.com/v3/business-mappings\nhumanURL: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas\nproperties:\n- url: https://www.ibm.com/docs/en/cloudability-commercial/cloudability-premium/saas\n  type: Documentation\ndescription: Business Mappings define rule-based dimensions that allocate spend to cost centers, products,\n  environments, or applications. The API lets callers list, create, update and delete mappings, manage\n  rule order and statements, and preview the resulting allocation against billing data.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cloudability/refs/heads/main/apis.yml
tags:
- Allocation
- Business Mappings
- Showback
- Tagging
---
