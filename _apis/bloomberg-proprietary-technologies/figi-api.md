---
aid: bloomberg-proprietary-technologies:figi-api
baseURL: https://api.openfigi.com
description: The Financial Instrument Global Identifier (FIGI) is Bloomberg's open standard for identifying financial instruments. The OpenFIGI API allows free mapping of tickers, ISINs, CUSIPs, and other identifiers to FIGI codes.
humanURL: https://www.openfigi.com/api
image: ''
layout: api
name: Bloomberg FIGI API
properties:
- type: Documentation
  url: https://www.openfigi.com/api
- type: OpenAPI
  url: https://api.openfigi.com/schema/openapi.json
provider_name: Bloomberg Proprietary Technologies
provider_slug: bloomberg-proprietary-technologies
slug: figi-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: bloomberg-proprietary-technologies:figi-api\nname: Bloomberg FIGI API\ndescription: The Financial Instrument Global Identifier (FIGI) is Bloomberg's open standard for identifying\n  financial instruments. The OpenFIGI API allows free mapping of tickers, ISINs, CUSIPs, and other identifiers\n  to FIGI codes.\nhumanURL: https://www.openfigi.com/api\nbaseURL: https://api.openfigi.com\ntags:\n- FIGI\n- Financial Identifiers\n- OpenFIGI\n- Mapping\nproperties:\n- type: Documentation\n  url: https://www.openfigi.com/api\n- type: OpenAPI\n  url: https://api.openfigi.com/schema/openapi.json\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/bloomberg-proprietary-technologies/refs/heads/main/apis.yml
tags:
- FIGI
- Financial Identifiers
- OpenFIGI
- Mapping
---
