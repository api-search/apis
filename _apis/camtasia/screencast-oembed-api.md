---
aid: camtasia:screencast-oembed-api
baseURL: https://app.screencast.com/services/oembed
description: Public oEmbed API for TechSmith Screencast (app.screencast.com), the cloud destination where Camtasia videos and images are shared. The oEmbed endpoint returns embed HTML, thumbnail, and metadata for a given Screencast content URL, letting content platforms and CMSes render Screencast shares inline the same way they render YouTube or Vimeo. Documentation is maintained in a public GitHub repository.
humanURL: https://github.com/TechSmith/screencast-public-api-docs
image: ''
layout: api
name: TechSmith Screencast oEmbed API
properties:
- type: Documentation
  url: https://github.com/TechSmith/screencast-public-api-docs/blob/main/sections/oembed.md
- type: Repository
  url: https://github.com/TechSmith/screencast-public-api-docs
- type: JSON-LD
  url: https://raw.githubusercontent.com/api-evangelist/camtasia/refs/heads/main/json-ld/camtasia-context.jsonld
provider_name: Camtasia
provider_slug: camtasia
slug: screencast-oembed-api
source_yaml: "aid: camtasia:screencast-oembed-api\nname: TechSmith Screencast oEmbed API\ndescription: Public oEmbed API for TechSmith Screencast (app.screencast.com), the cloud destination where\n  Camtasia videos and images are shared. The oEmbed endpoint returns embed HTML, thumbnail, and metadata\n  for a given Screencast content URL, letting content platforms and CMSes render Screencast shares inline\n  the same way they render YouTube or Vimeo. Documentation is maintained in a public GitHub repository.\nhumanURL: https://github.com/TechSmith/screencast-public-api-docs\nbaseURL: https://app.screencast.com/services/oembed\ntags:\n- oEmbed\n- Screencast\n- Embedding\n- Video\nproperties:\n- type: Documentation\n  url: https://github.com/TechSmith/screencast-public-api-docs/blob/main/sections/oembed.md\n- type: Repository\n  url: https://github.com/TechSmith/screencast-public-api-docs\n- type: JSON-LD\n  url: json-ld/camtasia-context.jsonld\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/camtasia/refs/heads/main/apis.yml
tags:
- oEmbed
- Screencast
- Embedding
- Video
---
