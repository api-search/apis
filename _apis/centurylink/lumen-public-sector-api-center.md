---
aid: centurylink:lumen-public-sector-api-center
baseURL: ''
description: The Public Sector API Center offers government-tailored REST APIs for Lumen network services, enabling federal, state, and local agencies to programmatically order, provision, and monitor connectivity and managed services.
humanURL: https://blog.centurylink.com/lumen-launches-public-sector-api-marketplace-which-offers-easy-to-use-apis/
image: ''
layout: api
name: Lumen Public Sector API Center
properties:
- type: Announcement
  url: https://blog.centurylink.com/lumen-launches-public-sector-api-marketplace-which-offers-easy-to-use-apis/
- type: Solutions
  url: https://www.lumen.com/en-us/solutions/public-sector.html
provider_name: CenturyLink (Lumen Technologies)
provider_slug: centurylink
slug: lumen-public-sector-api-center
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: centurylink:lumen-public-sector-api-center\nname: Lumen Public Sector API Center\ntags:\n- Federal\n- Government\n- Public Sector\n- State and Local\nhumanURL: https://blog.centurylink.com/lumen-launches-public-sector-api-marketplace-which-offers-easy-to-use-apis/\nproperties:\n- url: https://blog.centurylink.com/lumen-launches-public-sector-api-marketplace-which-offers-easy-to-use-apis/\n  type: Announcement\n- url: https://www.lumen.com/en-us/solutions/public-sector.html\n  type: Solutions\ndescription: The Public Sector API Center offers government-tailored REST APIs for Lumen network services,\n  enabling federal, state, and local agencies to programmatically order, provision, and monitor connectivity\n  and managed services.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/centurylink/refs/heads/main/apis.yml
tags:
- Federal
- Government
- Public Sector
- State and Local
---
