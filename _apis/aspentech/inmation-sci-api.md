---
aid: aspentech:inmation-sci-api
baseURL: http://hostname:8002
description: The AspenTech Inmation Simple Call Interface (SCI) API provides a simplified HTTP interface for communicating with the Inmation industrial data platform. Designed for straightforward read/write access to process data and configuration items in manufacturing and energy environments.
humanURL: https://atdocs.inmation.com/api/1.102/sci/index.html
image: ''
layout: api
name: AspenTech Inmation Simple Call Interface (SCI) API
properties:
- type: Documentation
  url: https://atdocs.inmation.com/api/1.102/sci/index.html
provider_name: AspenTech
provider_slug: aspentech
slug: inmation-sci-api
source_yaml: "aid: aspentech:inmation-sci-api\nname: AspenTech Inmation Simple Call Interface (SCI) API\ndescription: The AspenTech Inmation Simple Call Interface (SCI) API provides a simplified HTTP interface\n  for communicating with the Inmation industrial data platform. Designed for straightforward read/write\n  access to process data and configuration items in manufacturing and energy environments.\nhumanURL: https://atdocs.inmation.com/api/1.102/sci/index.html\nbaseURL: http://hostname:8002\ntags:\n- Industrial IoT\n- Manufacturing\n- Process Optimization\n- REST\nproperties:\n- type: Documentation\n  url: https://atdocs.inmation.com/api/1.102/sci/index.html\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/aspentech/refs/heads/main/apis.yml
tags:
- Industrial IoT
- Manufacturing
- Process Optimization
- REST
---
