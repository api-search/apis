---
aid: angular-14:angular-service-worker-api
baseURL: https://angular.io/api/service-worker
description: Angular 14 service worker support for building Progressive Web Applications with offline capabilities and push notification support.
humanURL: https://v14.angular.io/api/service-worker
image: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: Angular 14 Service Worker API
properties:
- type: Documentation
  url: https://v14.angular.io/api/service-worker
- type: Guide
  url: https://v14.angular.io/guide/service-worker-intro
provider_name: Angular 14
provider_slug: angular-14
slug: angular-service-worker-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: angular-14:angular-service-worker-api\nname: Angular 14 Service Worker API\ntags:\n- Offline\n- PWA\n- Service Worker\nimage: https://kinlane-productions2.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nbaseURL: https://angular.io/api/service-worker\nhumanURL: https://v14.angular.io/api/service-worker\nproperties:\n- url: https://v14.angular.io/api/service-worker\n  type: Documentation\n- url: https://v14.angular.io/guide/service-worker-intro\n  type: Guide\ndescription: Angular 14 service worker support for building Progressive Web Applications with offline\n  capabilities and push notification support.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/angular-14/refs/heads/main/apis.yml
tags:
- Offline
- PWA
- Service Worker
---
