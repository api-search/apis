---
aid: berkshire-hathaway:bnsf-api
baseURL: ''
description: The BNSF API Center provides customer APIs for programmatic integration with BNSF Railway freight shipping operations. APIs enable real-time shipment tracing, intermodal hub operations, pricing and rate retrieval, transit schedule lookups, waybill management, and diagnostic testing. Authentication uses certificate-based mutual TLS (mTLS) for secure identity validation between customers and BNSF systems.
humanURL: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/
image: ''
layout: api
name: BNSF Railway API
properties:
- type: Documentation
  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/
- type: GettingStarted
  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/getting-started/
- type: Console
  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/developers-console/
- type: APIReference
  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/catalog/
- type: Support
  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/support/
provider_name: Berkshire Hathaway
provider_slug: berkshire-hathaway
slug: bnsf-api
source_filename: apis.yml
source_heading: API entry from apis.yml
source_yaml: "aid: berkshire-hathaway:bnsf-api\nname: BNSF Railway API\ntags:\n- Freight\n- Logistics\n- Rail\n- Shipment Tracking\n- Supply Chain\n- Transportation\nhumanURL: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/\ndescription: The BNSF API Center provides customer APIs for programmatic integration with BNSF Railway\n  freight shipping operations. APIs enable real-time shipment tracing, intermodal hub operations, pricing\n  and rate retrieval, transit schedule lookups, waybill management, and diagnostic testing. Authentication\n  uses certificate-based mutual TLS (mTLS) for secure identity validation between customers and BNSF systems.\nproperties:\n- type: Documentation\n  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/\n- type: GettingStarted\n  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/getting-started/\n- type: Console\n  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/developers-console/\n\
  - type: APIReference\n  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/catalog/\n- type: Support\n  url: https://www.bnsf.com/ship-with-bnsf/support-services/customer-api/support/\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/berkshire-hathaway/refs/heads/main/apis.yml
tags:
- Freight
- Logistics
- Rail
- Shipment Tracking
- Supply Chain
- Transportation
---
