---
aid: cdw:cdw-eprocurement-api
baseURL: ''
description: CDW eProcurement integration supports PunchOut and Roundtrip catalogs via cXML or OCI, electronic purchase order submission via cXML, EDI, or flat file, and electronic invoicing via XML or EDI. Orders can be submitted via HTTP, SFTP, API, and EDI connections.
humanURL: https://www.cdw.com/content/cdw/en/services/eprocurement-and-custom-catalogs.html
image: https://kinlane-productions.s3.amazonaws.com/apis-json/apis-json-logo.jpg
layout: api
name: CDW eProcurement Integration
properties:
- type: Documentation
  url: https://www.cdw.com/content/cdw/en/services/eprocurement-and-custom-catalogs.html
provider_name: CDW
provider_slug: cdw
slug: cdw-eprocurement-api
source_yaml: "aid: cdw:cdw-eprocurement-api\nname: CDW eProcurement Integration\ntags:\n- EDI\n- eProcurement\n- PunchOut\n- cXML\nimage: https://kinlane-productions.s3.amazonaws.com/apis-json/apis-json-logo.jpg\nhumanURL: https://www.cdw.com/content/cdw/en/services/eprocurement-and-custom-catalogs.html\nproperties:\n- url: https://www.cdw.com/content/cdw/en/services/eprocurement-and-custom-catalogs.html\n  type: Documentation\ndescription: CDW eProcurement integration supports PunchOut and Roundtrip catalogs via cXML or OCI, electronic\n  purchase order submission via cXML, EDI, or flat file, and electronic invoicing via XML or EDI. Orders\n  can be submitted via HTTP, SFTP, API, and EDI connections.\n"
source_yaml_url: https://raw.githubusercontent.com/api-evangelist/cdw/refs/heads/main/apis.yml
tags:
- EDI
- eProcurement
- PunchOut
- cXML
---
